    ...
    {author.length > 0 ? ( // Only render authors if there are any
        <ul className='text-surface dark:text-white'>
          ...
          {author.map((author, index) => (
            <li className='w-full cursor-default border-b-2 border-neutral-100 py-4 text-surface/50 dark:border-white/10 dark:text-white/50 dark:md:hover:bg-sky-900' key={index}><a className='bg-sky-1000' href="">{author}</a></li>
          ))}
